...
public class TufRepositoryTests
{
    ...
    {
        // Use shared test repository to avoid expensive key generation and repository building
        return TestOptimizations.SharedTestRepository.Value;
    }
    ...
    public async Task WriteToDirectory_ValidPath_CreatesCorrectStructure()
    {
        ...
        using var cleanup = TestOptimizations.CreateManagedTestDirectory(out var tempDir);
    ...
    public async Task WriteToDirectory_NestedTargetPaths_CreatesNestedDirectories()
    {
        var builder = new RepositoryBuilder()
            .AddSigner("root", TestOptimizations.SharedSigners.Root)
            .AddSigner("timestamp", TestOptimizations.SharedSigners.Timestamp)
            .AddSigner("snapshot", TestOptimizations.SharedSigners.Snapshot)
            .AddSigner("targets", TestOptimizations.SharedSigners.Targets)
