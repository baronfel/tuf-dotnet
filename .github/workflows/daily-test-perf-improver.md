...
# Daily Test and Build Perf Improver
...
1. Performance research (if not done before).
   ...
   1b. Do some deep research into the performance of the tests and build of this repo.
     - How can test and build performance be measured?
     - What are the normal build commands used by local development, as well as CI/CD workflows
     - What is the structure of the repo layout, and how does MSBuild process the projects and their relationships?

     Research:
     - Any causes of additional builds, unnecessary builds, or slow build steps in general
     - Test cases that are slow and ways the tests could be re-factored or reuse cached data to run faster
     - Features of MSbuild or the .NET SDK that can lead to faster build times
