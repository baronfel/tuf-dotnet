...
public class TrustedMetadata
{
    ...
    public virtual TrustedMetadataWithTimestamp UpdateTargets(byte[] targetsData)
    {
        throw new InvalidOperationException("Cannot update targets before snapshot");
    }
    ...
    : TrustedMetadataWithTimestamp(root, timestamp, refTime)
{
    ...
    /// <summary>
    /// Updates the top-level targets metadata.
    /// </summary>
    /// <param name="targetsData">JSON bytes of targets metadata</param>
    /// <returns>Complete trusted metadata with targets information</returns>
    public override CompleteTrustedMetadata UpdateTargets(byte[] targetsData)
    {
        return (CompleteTrustedMetadata)UpdateDelegatedTargets(targetsData, "targets", "root");
    }
